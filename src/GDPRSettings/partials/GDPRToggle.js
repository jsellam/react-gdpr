      ...
      return (
        <div className="gdpr-toggle validated">
          <div className="gdpr-status gdpr-toggle-status">
            {this.props.locale.gdpr_always_active}
          </div>
        </div>
      );
    }
    let label = this.props.isActive
      ? this.props.locale.gdpr_active
      : this.props.locale.gdpr_inactive;
    return (
      <div
        onClick={this.handleClick}
        className={classNames('gdpr-toggle', this.props.className, {
          validated: this.props.isActive
        })}
      >
        <div className="gdpr-status gdpr-toggle-status">{label}</div>
        <div className="gdpr-toggle-track">
          {' '}
          <div className="gdpr-toggle-cursor" />
        </div>
      </div>
    );
  }
